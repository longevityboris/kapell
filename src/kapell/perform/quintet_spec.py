...
# fixture layout relative to the working directory.
SCORE = Path(sys.argv[1]) if len(sys.argv) > 1 else Path("score/music-voices.ly")
OUT = Path(sys.argv[2]) if len(sys.argv) > 2 else Path("performance/quintet/ricercar_quintet.json")
...
def main():
    ...
    print(f"wrote {OUT}: {len(assignments)} windows, {len(spans)} pedal spans")
    for sp in spans:
