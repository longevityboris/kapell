...
python3 ../../perform/perform.py "$ROOT/fugue.ly" demo/fugue_plan.json out/demo/fugue_strings.mid --target strings
...
if [[ -f "$FL/SK_final.ly" && -f "$FL/plan.json" ]]; then
  python3 ../../perform/perform.py "$FL/SK_final.ly" "$FL/plan.json" out/demo/sk_final_strings.mid --target strings
